    ...
    constructor(props) {
     ...
     cycle() {
        ...
        if (!this.state.mouseInside) {
            this.setState({ activeIndex: (this.state.activeIndex + 1) % this.state.slides.length });
        }
    }

     setActive(idx) {
        clearInterval(this.state.intervalFunction);
        this.setState({ activeIndex: idx - 1, intervalFunction: setInterval(this.cycle, 5000) });
    }
     ...
     render() {
        return (
            <div className="carousel-container">
                <CarouselSelector selected={this.state.activeIndex} itemCount={this.state.slides.length}
                    selectHandler={this.setActive} />
                <div className="carousel-content" onMouseEnter={this.onMouseEnter} onMouseLeave={this.onMouseLeave}>
