            ...
            <li key={page.position} className={`menu ${styleName || ""}`}>
              ...
              <ul className={`menu ${styleName || ""}`}>
                {page.subpages.map(subpage => {
                  return (
                    subpage.tags.some(element =>
                      currentTags.includes(element)
                    ) && ( //tags array comparison to show the proper sub-menu list
                      <Link
                        activeClassName="active"
                        to={`/${subpage.uid}`}
                        key={subpage.title}
                    ...
                    ) // the currentTag comes from Service component
                  )
                })}
              </ul>
            </li>
...
export default props => (
  <StaticQuery
    ...
    render={data => {
      ...
      return <Menu data={mainPages} {...props} />
    }}
  />
)
