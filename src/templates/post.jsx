...
const Post = ({ data: { prismicPost, allPrismicPost } }) => {
  ...
  const currentIndex = allPrismicPost.edges.findIndex(
    el => el.node.uid === prismicPost.uid
  )
  const prevIndex = currentIndex + 1
  const nextIndex = currentIndex - 1
  const prevSlug =
    prevIndex >= allPrismicPost.edges.length
      ? null
      : allPrismicPost.edges[prevIndex].node.uid
  const nextSlug =
    nextIndex < 0 ? null : allPrismicPost.edges[nextIndex].node.uid
  return (
    <>
      <Header />
      <Container>
        ...
        <Content>
          ...
          <span>
            {moment(`${data.date}`).format("DD MMMM YYYY")} - skrevet av{" "}
            {data.author}
          </span>
          ...
          <div style={{ textAlign: "left", paddingTop: "20px" }}>
            ...
            >
              <FacebookShareButton url={`http://ayr.no/${prismicPost.uid}`}>
